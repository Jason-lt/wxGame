...
hall.adManager = {
    ...
    showBigAd:function () {
        ...
        wx.previewImage({
            ...
            success:function(res){
                shot.LOGD(null, "预览图片成功！");
            },
            fail:function (res) {
                shot.LOGD(null, "预览图片失败！");
            }
        });
    },
    ...
    showRewardedVideo:function (adId,type) {
        ...
        if (!wx.hasOwnProperty('createRewardedVideoAd')){
            ...
            ty.NotificationCenter.trigger(shot.EventType.REWARD_VIDEO_COMPLETE_ERROR,'您当前版本不支持广告视频');
            return;
        }
        ...
        var onvVdClose = function (res) {
            ...
            ty.NotificationCenter.trigger(shot.EventType.REWARD_VIDEO_COMPLETE, adId);
