...
shot.Share = {
    ...
    shareImformation : function (type, shareCfg, titleString, imageUrl, query, sharePoint, successCallBackFun, failCallBackFun) {
        ...
        wx.shareAppMessage({
            ...
            success : function (result) {
                // hall.LOGE("shareAppMessage","===shareAppMessage==success=="+JSON.stringify(arguments));
            ...
            fail : function () {
                // hall.LOGE("shareAppMessage","===shareAppMessage==fail=="+JSON.stringify(arguments));
                if (failCallBackFun){
                    failCallBackFun();
                }
    ...
    playAnimationAfterShareWithType:function() {
        ...
        if(shot.Share.unSensitivePersonage == 0){
            // this.endWithSuccessShare();
            ty.NotificationCenter.trigger(shot.EventType.UPDATE_SHARE_STATE,shot.Share.shareType);
            return;
        }
        ...
        //分享点什么都不验证,直接发钻石
        if(shot.Share.unSensitivePersonage == 1){
            this.endWithSuccessShare();
            return;
        }
    ...
    getShareResultWithKey : function (result,shareKey) {
        hall.LOGW("","file = [Share] fun = [getShareResultWithKey]  ");
        var iv = result.iv ;
    ...
    endWithSuccessShare : function () {
        ...
        if(shot.Share.shareType != shot.Share.onShareType.clickStatShareTypeRevial &&
            shot.Share.shareType != shot.Share.onShareType.clickStatShareTypeOpenSecretBoxB &&
            shot.Share.shareType != shot.Share.onShareType.clickStatShareTypeOpenSecretBoxN){
    ...
    clickStatShareType :{
        clickStatShareTypeMysteryGiftBag:"214",//神秘礼包
        ...
        clickStatShareTypeMysteryGiftBagGetReward : "204",//分享得钻石
...
wx.onShareAppMessage(function (result) {
    ...
    return {
        ...
        success : function (shareTickets,groupMsgInfos) {
            hall.LOGE("onShareAppMessage","===onShareAppMessage==success=="+JSON.stringify(arguments));
            ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeShareAction,[sharePointId , 2, shareSchemeId]);
        },
        fail : function () {
            hall.LOGE("onShareAppMessage","===onShareAppMessage==fail=="+JSON.stringify(arguments));
        },
        complete : function () {
            hall.LOGE("onShareAppMessage","===onShareAppMessage==complete=="+JSON.stringify(arguments));
        }
    }
});
