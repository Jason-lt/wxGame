...
shot.GlobalFuncs.showAdBtnWithTag  = function (tag) {
	...
	if(!shot.gameModel.getAllCheckConfig()){
		...
		//     hall.adManager.showAd({x:72,y:sceneSize.height-350});
		// }else {
		//     hall.adManager.getAdNodeList()[0].y = sceneSize.height-200;
...
shot.GlobalFuncs.hideAdBtnWithTag  = function (tag) {
	if(hall.adManager.getAdNodeByTag(tag)){
