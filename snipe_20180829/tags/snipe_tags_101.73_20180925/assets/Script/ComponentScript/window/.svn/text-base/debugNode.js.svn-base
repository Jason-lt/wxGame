...
cc.Class({
    ...
    onLoad : function() {
        hall.GlobalFuncs.setInLocalStorage(snipe.gameModel.DEBUG_MODE,true);
    ...
    numberEditEnd : function (event) {
        ...
        if(changedString == '20180706'){
            hall.GlobalFuncs.setInLocalStorage(snipe.gameModel.DEBUG_MODE,false);
            this.node.destroy();
            return;
        }
    ...
    updateScoreCloud : function () {
        snipe.GameWorld.totalScore = this.numberEditBox.string;
        snipe.GlobalFuncs.upDateRankData(snipe.GameWorld.totalScore);
    },

    changeInitScore : function () {
        snipe.GameWorld.initGameLevel = parseInt(this.numberEditBox.string);
        // snipe.gameModel.initScore = parseInt(this.numberEditBox.string);
    },
    ...
    onUserLaser : function () {
        snipe.GameWorld.canUserLaser = true;
    },
    ...
    //     snipe.Share.shareWithType(this.txtShareType.string, null, null, shareCfg);
    // },
