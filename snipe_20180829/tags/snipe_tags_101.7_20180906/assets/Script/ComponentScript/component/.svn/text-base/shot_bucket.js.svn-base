...
cc.Class({
    ...
    setRotateTimeWithType : function (type) {
        ...
        if(type == "bottom" || type == "top"){
            if(type == "bottom"){
                timeConfig = snipe.GameWorld.rotationTimeConfig.bottomBucket;
            }else if(type == "top"){
                timeConfig = snipe.GameWorld.rotationTimeConfig.topBucket;
            }
            ...
            timeConfig = snipe.GameWorld.rotationTimeConfig.onlyBucket;
    ...
    setBottlesWithListNew : function (bottleList) {
        ...
        for(var i = 0 ; i < this.bottleList.length ; i ++){
            ...
            if(this.bottleList[i] != "none"){
                ...
                var rotation = i * 30;
                sectionR = snipe.GameWorld.bottleConfig[this.bottleList[i]].sectionR;
    ...
    receiveBulletNew : function (runningFire) {
        ...
        if(snipe.GameWorld.bulletRange){
            this.bottleAniNodeLeft.rotation = 180-snipe.GameWorld.bulletRange;
            this.bottleAniNodeRight.rotation = 180+snipe.GameWorld.bulletRange;
            var tempBottles = [];
            if(bottle){
            ...
            for (var j = 0 ; j < 2 ; j ++) {
                if (j == 0) {
                    rotation = 540 - this.roundRotation - snipe.GameWorld.bulletRange;
                } else {
                    rotation = 540 - this.roundRotation + snipe.GameWorld.bulletRange;
                }
                if (rotation < 0) {
        ...
        if(this.bgType == "only" || this.bgType == "bottom"){
            ...
            if(snipe.GameWorld.bulletRunningTimeTop) {
                snipe.GameWorld.bulletRunningTimeTop--;
                if (snipe.GameWorld.bulletRunningTimeTop) {
                    this.receiveBulletIntervalCount = snipe.GameWorld.bulletRunningInterval;
                    return;
                }
    ...
    //     // snipe.GameWorld.bulletRunningTime = 0;
    ...
    playDeadAnimation : function (range) {
        ...
        if(!range){
            ...
            if(snipe.GameWorld.bulletRange){
                this.playDeadAnimation(nowRotation-snipe.GameWorld.bulletRange);
                this.playDeadAnimation(nowRotation+snipe.GameWorld.bulletRange);
            }
        }else {
            nowRotation = range;
    ...
    receiveBottleLifeCount : function (bottleInfo) {
        ...
        if(bottleInfo.lifeCount == 0){
            if(bottleInfo.type.indexOf("treasureBox") > -1){
                ...
                snipe.GameWorld.gameGetBox = true;
                snipe.GameWorld.gunFireIntervalNow = 0;
                snipe.GlobalFuncs.showSecretBox(bottleInfo.type,this.parentScene);
            }
            if(!snipe.GameWorld.crazyMoment){
                this.bottleList[bottleInfo.sitNumber] = "none";
            }else {
                snipe.GameWorld.crazyBottleCount[bottleInfo.type] ++;
                // snipe.GameWorld.crazyBottleCount ++;
            }
            snipe.GameWorld.bottleCount --;
    ...
    setCrazyRotateTimeWithType : function (type,bucketConfig) {
        ...
        for (var key in bottleConfig){
            ...
            for (var i = 0; i < count ; i ++){
                ...
                sectionR = snipe.GameWorld.bottleConfig[key].sectionR;
                for (j = rotation-sectionR ; j < rotation + sectionR ; j ++){
                    var setIndex = j >=0 ? j : j+360;
                    if(setIndex >= 360){
                        setIndex -= 360;
            ...
            snipe.GameWorld.bottleCount += count;
        }
    },
    ...
    update : function(dt) {
        ...
        if(snipe.GameWorld.gameOver || snipe.GameWorld.gamePause || this.bgType == "none"){
            return;
        }
