...
ty.TuyooSDK = {
    ...
    //                                 },
    //                                 fail:function () {
    //                                     ty.NotificationCenter.trigger(snipe.EventType.START_AUTHORIZATION_FAILED);
    //                                     snipe.needReLogin = true;
    ...
    loginTuyooWithCode: function(code, userInfo, iv, encryptedData) {
        ...
        debugMode = hall.GlobalFuncs.ReadBoolFromLocalStorage(snipe.gameModel.DEBUG_MODE,debugMode);
        if (debugMode){
        ...
        // snipe.Share.shareKeywordReplace.wechatName = userInfo.nickName;
        ...
        try {
            wx.request({
                ...
                success: function(params) {
                    ...
                    snipe.Share.shareKeywordReplace.nickName = hall.GlobalFuncs.sliceStringToLength(ty.UserInfo.userName,10);
    ...
    //             ty.Timer.cancelTimer(cc.director, checkUserInfo);
    //             snipe.GameWorld.game_friendData = context['game_friendData'];
    //
    //         }
    //     };
    //     ty.Timer.setTimer(cc.director, checkUserInfo, 1 / 60);
...
wx.onShow(function (result) {
    ...
    if(query && query.sourceCode){ //从小程序消息卡片中点入
        ...
        if (query && query.propBoxID) {
            snipe.Share.shareKeywordReplace.boxId = query.propBoxID;
            ty.SystemInfo.boxId = query.propBoxID;
        }

        if (query && query.giftBagBoxId) {
            snipe.Share.shareKeywordReplace.mysteryGiftBagBoxId = query.giftBagBoxId;
            ty.SystemInfo.mysteryGiftBagBoxId = query.giftBagBoxId;
        }
    }

    if (snipe.needReLogin){
        hall.LoginToyoo();
        snipe.needReLogin = null;
    }

    // if(snipe.Share.isOnShare && ty.UserInfo.userId){
