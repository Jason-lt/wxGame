...
//
// //如果文件里存了音量信息，则进行设置
// var mute = hall.GlobalFuncs.ReadBoolFromLocalStorage(snipe.SETTING_MUTE_KEY, false);
// if (mute) {
//     snipe.AudioHelper.setMusicVolume(0);
//     snipe.AudioHelper.setEffectsVolume(0);
// } else {
//     var music_vol = hall.GlobalFuncs.ReadNumFromLocalStorage(snipe.SETTING_MUSIC_KEY, 0.5); //如果没有存储，初始默认值0.5
//     snipe.AudioHelper.setMusicVolume(music_vol);
//     var effect_vol = hall.GlobalFuncs.ReadNumFromLocalStorage(snipe.SETTING_EFFECT_KEY, 1); //如果没有存储，初始默认值1
//     snipe.AudioHelper.setEffectsVolume(effect_vol);
// }
                    ...
                    wx.getSetting({
                        success:function(res) {
                            if (!res.authSetting['scope.userInfo']) {
                                ...
                                var openData = snipe.GlobalFuncs.getOpenData();
                                if (openData){
