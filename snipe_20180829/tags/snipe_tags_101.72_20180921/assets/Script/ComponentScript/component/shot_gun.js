...
cc.Class({
    ...
    onLoad : function() {
        ...
        if(!snipe.GameWorld.gunnerShareSchemeConfig || snipe.GameWorld.gunnerShareSchemeConfig.showBanner){
            this.grenadeAniNode.y = 183;
        }else {
            this.grenadeAniNode.y = 293;
        }

        ty.NotificationCenter.listen(snipe.EventType.REPLACE_GUN,this.replaceGunAction, this);

        if(snipe.GameWorld.initBulletType){
            var index = snipe.GameWorld.bulletTypeList.indexOf(snipe.GameWorld.initBulletType);
            this.replaceGunAction(index);
        }
    },
    ...
    openFire : function () {
        snipe.GameWorld.nowBulletPenetrate = snipe.GameWorld.bulletPenetrate;//穿透属性
        snipe.GameWorld.grenadeRunning = false;
        // hall.LOGW("===","========openFire=====");
        snipe.AudioHelper.playEffect(snipe.EffectPath_mp3.gunHits,false);
        this.playGunAni();
    },
    ...
    onActionGrenade: function(callFunc) {
        ...
        var gStartFunc = function (_preFabNode,_callFunc){
            ...
            var strikeSound = cc.callFunc(function () {
                snipe.AudioHelper.playEffect(snipe.EffectPath_mp3.grenadeStrike);
            });
            ...
            _preFabNode.runAction(cc.sequence(cc.spawn(rotationAct,bezierToDst1),cc.spawn(rotationAct2,strikeSound), cc.callFunc(function () {
                ...
                snipe.AudioHelper.playEffect(snipe.EffectPath_mp3.grenadeBoom);
