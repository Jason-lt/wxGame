...
hall.adManager = {
    ...
    videoAdId : "adunit-5da392d11055a6a5",
    rewardedVideoType:"",
    videoAdManager : null,
    ...
    showRewardedVideo:function (type) {
        ...
        if (!wx.hasOwnProperty('createRewardedVideoAd')){
            ...
            ty.NotificationCenter.trigger(snipe.EventType.REWARD_VIDEO_COMPLETE_ERROR,'您的微信版本过低,请升级至6.6.6以上');
            return;
        }
        ...
        var onvVdClose = function (res) {
            ...
            //只有播放完成才发奖
            ty.NotificationCenter.trigger(snipe.EventType.REWARD_VIDEO_COMPLETE, playEnded);
        ...
        if (hall.adManager.canPlay){
            ...
            ty.NotificationCenter.trigger(snipe.EventType.REWARD_VIDEO_COMPLETE, true);
        }
        ...
        //         }
        //         ty.NotificationCenter.trigger(snipe.EventType.REWARD_VIDEO_COMPLETE_ERROR, err.errMsg);
        //         snipe.LOGD(null, err.errMsg)
        //     });
        // });
    },
    ...
    showBannerAd:function (adid,refresh) {
        ...
        var top = 0;
        if(!snipe.curBannerAd){
            snipe.curBannerAd = wx.createBannerAd({
                adUnitId: adid,
                style: {
                    left:0,
                    top:top,
        ...
        //     }
        // });
        snipe.curBannerAd.onError(function (err) {
            hall.adManager.showBanner({x:cc.director.getWinSize().width/2,y:83});
            hall.LOGW('snipe.curBannerAd.onError:' + err);
        });

        snipe.curBannerAd.onResize(function (res) {
            hall.LOGW('showBannerAd', '当前banner,width:' + res.width + "; height:" + res.height);
            if(snipe.curBannerAd){
                // snipe.curBannerAd.style.left = (screenWidth - res.width)/2;
                // snipe.curBannerAd.style.left = 0;
                // snipe.curBannerAd.style.top = screenHeight - res.height + 1+75;
                // snipe.curBannerAd.style.top = screenHeight - 80;
        ...
        snipe.curBannerAd.show();
    },

    hideBannerAd : function () {
        if(snipe.curBannerAd){
            snipe.curBannerAd.hide();
        }
    },

    destroyBannerAd:function () {
        if (snipe.curBannerAd){
            snipe.curBannerAd.destroy();
            snipe.curBannerAd = null;
        }
    },
    ...
    adNodeObj  : {
        ...
        createAdNode : function (pos) {
            ...
            if(this.adIconBtn) {
                ...
                cc.loader.loadRes('snipe_prefabs/adNode', function (err, prefab) {
        ...
        onClickAdIconBtn: function() {
            hall.GlobalFuncs.setInLocalStorage(snipe.gameModel.OPEN_ADBTN_TYPE_TODAY,'CO');
