...
scratch.NetWorkCenter = {
    canNotLeave : false,

    boot : function () {
        this._TAG = 'scratch_NetWorkCenter';
        hall.LOGD(this._TAG, 'boot');

        this.cmdMap = {};

        this.cmdMap[scratch.EventType.MSG_GAME] = this.onGame;
        this.cmdMap[scratch.EventType.MSG_CASH] = this.onCash;
        // this.cmdMap[scratch.EventType.MSG_CASH] = this.onStoreExchange;
        ...
        this.cmdMap[scratch.EventType.HALL_SHARE2] = this.onShare2;
    ...
    onShare2:function (msg) {
        hall.LOGW("","file = [scratchNetWrokCenter] fun = [onShare2] msg = " + JSON.stringify(msg));
        if (msg.result.action == scratch.EventType.GET_REWARD){
            // {"cmd":"hall_share2","result":{"action":"get_reward","gameId":6,"userId":10008,"rewards":[]}}
            //获得奖励回调
            if (scratch.waitGetRevial && scratch.waitGetRevial.type == 'waitRecive' && msg.result.rewards.length > 0){
                // scratch.gameModel.consumeAssets(1,"item:1373");
                scratch.waitGetRevial = null;
                ...
                if (msg.result.rewards[0].count) {
                    ...
                    if (msg.result.rewards[0].itemId == "item:1371") {
                        scratch.GlobalFuncs.playGetPropBoxAni(0);
                    }else if (msg.result.rewards[0].itemId == "item:1372") {
                        scratch.GlobalFuncs.playGetPropBoxAni(1);
                    }else if (msg.result.rewards[0].itemId == "item:1390") {
                        scratch.GlobalFuncs.playGetPropBoxAni(8);
        ...
        else if (msg.result.action == scratch.EventType.CHECK_REWARD){
            //检查当前分享点是否还有奖励
    ...
    onGame: function(argument) {
        scratch.gameModel.parseGame(argument);
    },
    onCash : function (argument) {
        scratch.gameModel.parseCash(argument);
    }
