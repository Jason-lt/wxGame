...
ty.TuyooSDK = {
    ...
    //                                 },
    //                                 fail:function () {
    //                                     ty.NotificationCenter.trigger(scratch.EventType.START_AUTHORIZATION_FAILED);
    //                                     scratch.needReLogin = true;
    ...
    wechatLoginDirect: function(loginCode) {
        ty.ServerStateManager.checkServerState(function () {
            ...
            //     scratch.GlobalFuncs.getUserInfo();
    ...
    loginTuyooWithCode: function(code, userInfo, iv, encryptedData) {
        ...
        // debugMode = hall.GlobalFuncs.ReadBoolFromLocalStorage(scratch.gameModel.DEBUG_MODE,debugMode);
        if (debugMode){
        ...
        // scratch.Share.shareKeywordReplace.wechatName = userInfo.nickName;
        ...
        try {
            wx.request({
                ...
                success: function(params) {
                    ...
                    scratch.Share.shareKeywordReplace.nickName = hall.GlobalFuncs.sliceStringToLength(ty.UserInfo.userName,10);
...
wx.onShow(function (result) {
    ...
    if(query && query.sourceCode){ //从小程序消息卡片中点入
        if(query.sourceCode == scratch.Share.onShareType.clickStatShareTypeRankList && scene == 1044){ //群排行榜
        ...
        if (query && query.propBoxID) {
            scratch.Share.shareKeywordReplace.boxId = query.propBoxID;
            ty.SystemInfo.boxId = query.propBoxID;
        }

        if (query && query.giftBagBoxId) {
            scratch.Share.shareKeywordReplace.mysteryGiftBagBoxId = query.giftBagBoxId;
            ty.SystemInfo.mysteryGiftBagBoxId = query.giftBagBoxId;
        }
    }

    if (scratch.needReLogin){
        hall.LoginToyoo();
        scratch.needReLogin = null;
    }

    // if(scratch.Share.isOnShare && ty.UserInfo.userId){
