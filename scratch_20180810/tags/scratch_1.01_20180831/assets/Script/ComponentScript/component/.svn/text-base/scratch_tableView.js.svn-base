...
cc.Class({
    ...
    setDataArray: function (dataArray) {
        ...
        if(scratch.scrollpos){
            this.scrollView.setContentPosition(scratch.scrollpos);
            this.scrollViewisScrolling(this.scrollView);
        }
        else{
    ...
    makeCellWithIndex : function (index) {
        ...
        this.cellList.push(ddz_cell);
    },

    removeAllCell : function () {
        for (var i = this.cellList.length-1 ; i >= 0; i --){
            if(this.cellList[i]){
                this.cellPool.put(this.cellList[i]);
                this.cellList[i].removeFromParent();
    ...
    deleteCellWithCell : function (cell) {
        if(cell){
            ...
            this.cellList.splice(this.cellList.indexOf(cell),1);
        }
    },
    scrollViewisScrolling : function (scrolView) {
        ...
        if (nowY+1000 > this.contentHeight){
            // ddz.LOGD(null, "============================================="+nowY);
            // ty.NotificationCenter.trigger(hall.EventType.SCROLLTOBOTTOM);
        }
        ...
        if (starIn < this.startIndex){
            if (starIn < this.startIndex - this.allLong ){
                for (var i = this.startIndex ;i <= this.startIndex + this.allLong ;i ++){
                    var cell = this.scrollView.content.getChildByName((i+500)+"");
                    this.deleteCellWithCell(cell);
                }
        ...
        scratch.scrollpos = this.scrollView.getContentPosition();
    },
