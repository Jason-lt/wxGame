...
hall.hallNetWorkCenter = {
    ...
    boot : function (){
        ...
        this.cmdMap[scratch.EventType.MSG_GAME_DATA] = this.onReceiveGameData;
        this.cmdMap[scratch.EventType.MSG_USER_INFO] = this.onReceiveUserInfo;
        this.cmdMap[scratch.EventType.MSG_BAG_INFO] = this.onReceiveBagInfo;
        this.cmdMap[scratch.EventType.MSG_DATA_CHANGED] = this.onUpdateChangedData;
        ty.NotificationCenter.listen(scratch.EventType.GET_USER_FEATURE_SUCCESS, this._userFeature, this);

        this.cmdMap[scratch.EventType.MSG_STORE_EXCHANGE] = this.onStoreExchange;
    },
    ...
    onGameData: function(argument) {
        ...
        if (typeof(argument) != "undefined") {
            if (!hall.GlobalFuncs.checkMsgWithGameId(argument, scratch.GameId)) {
                // 大厅的game_data，返回，暂时没内容
                return;
            }
            hall.MsgFactory.getUserInfo(scratch.GameId);
        }
    },
    _userFeature:function (val) {
        ...
        // ty.NotificationCenter.trigger(scratch.EventType.BSGS_CHECK_RESULT);
    },
    onReceiveUserInfo : function(value){
        ...
        // scratch.gameModel.getNormalConfig();
        // scratch.gameModel.getUserInfoForShot();
        // scratch.gameModel.getShareConfig();
        // scratch.gameModel.getGunnerShareSchemeConfig();
        ...
        if (scratch.qrCodeOption.withQrCode(scene)){
            //是否为从小程序码进入
            var par = "";
            ...
            scratch.qrCodeOption.runOption(par, scene);
        }
        if(query && query.sourceCode){//从小程序消息卡片中点入
            ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeUserFrom,[scene,query.inviteCode,scratch.Share.clickStatShareType[query.sourceCode],
                query.imageType,"GameStart",JSON.stringify(sceneInfo)]);
            if (query.inviteCode) {
                scratch.gameModel.bindInvite(parseInt(query.inviteCode));
            }
            ...
            if (query.inviteCode && shareTicket) {
                ...
                scratch.Share.getOpenGid(shareTicket,_boxUserId);
            }
        }else {
        ...
        //     ty.NotificationCenter.trigger(scratch.EventType.REPLAY_BG_MUSIC);
        // });
        // ty.NotificationCenter.trigger(scratch.EventType.REPLAY_BG_MUSIC);

        requestAnimationFrame(function () {
            if(scratch.Share.isOnShare){
                scratch.Share.playAnimationAfterShareWithType();
            }
            scratch.Share.isOnShare = false;
        });

        //
        // if (ty.SystemInfo.shareTicket &&  ty.SystemInfo.shareTicket != ""){
        //     var shareTicket = ty.SystemInfo.shareTicket;
        //     scratch.GlobalFuncs.showRankList(shareTicket);
        // }
    ...
    onStoreExchange : function (argument) {
        // hall.LOGD("hello","parseGame");
        scratch.gameModel.parseStoreExchange(argument);
    }
};
