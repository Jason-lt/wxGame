...
hall.adManager = {
    ...
    showRewardedVideo:function (type) {
        ...
        if (!wx.hasOwnProperty('createRewardedVideoAd')){
            ...
            ty.NotificationCenter.trigger(scratch.EventType.REWARD_VIDEO_COMPLETE_ERROR,'您的微信版本过低,请升级至6.6.6以上');
            return;
        }
        ...
        var onvVdClose = function (res) {
            ...
            //只有播放完成才发奖
            ty.NotificationCenter.trigger(scratch.EventType.REWARD_VIDEO_COMPLETE, playEnded);
        ...
        if (hall.adManager.canPlay){
            ...
            ty.NotificationCenter.trigger(scratch.EventType.REWARD_VIDEO_COMPLETE, true);
        }
        ...
        //         }
        //         ty.NotificationCenter.trigger(scratch.EventType.REWARD_VIDEO_COMPLETE_ERROR, err.errMsg);
        //         hall.LOGD(null, err.errMsg)
        //     });
        // });
    },
    ...
    showBannerAd:function (adid) {
        if(!scratch.GameWorld.gunnerShareSchemeConfig || !scratch.GameWorld.gunnerShareSchemeConfig.showBanner){
            return;
        }
        ...
        var top = 0;
        scratch.curBannerAd = wx.createBannerAd({
            adUnitId: adid,
            style: {
                left:0,
                top:top,
        ...
        scratch.curBannerAd.onResize(function (res) {
            hall.LOGW('showBannerAd', '当前banner,width:' + res.width + "; height:" + res.height);
            if(scratch.curBannerAd){
                // scratch.curBannerAd.style.left = (screenWidth - res.width)/2;
                scratch.curBannerAd.style.left = 0;
                // scratch.curBannerAd.style.top = screenHeight - res.height + 1+75;
                // scratch.curBannerAd.style.top = screenHeight - 80;
                // top = res.height > 92 ? 92 : res.height;
                top = res.height;
                if(ty.UserInfo.systemType == ty.UserInfo.SYSTEMTYPE.iPhoneXType){
                    scratch.curBannerAd.style.top = screenHeight- top - 30;
                }else {
                    scratch.curBannerAd.style.top = screenHeight- top;
                }
            }
        });

        scratch.curBannerAd.show();
    },

    destroyBannerAd:function () {
        if (scratch.curBannerAd){
            scratch.curBannerAd.destroy();
            scratch.curBannerAd = null;
        }
    },
    ...
    adNodeObj  : {
        ...
        createAdNode : function (pos) {
            ...
            if(this.adIconBtn) {
                ...
                cc.loader.loadRes('scratch_prefabs/adNode', function (err, prefab) {
                    var preFabNode = cc.instantiate(prefab);
                    preFabNode.position = cc.p(pos.x, pos.y);
                    that.adIconBtn = preFabNode;
                    // scratch.adIconBtn = preFabNode;
