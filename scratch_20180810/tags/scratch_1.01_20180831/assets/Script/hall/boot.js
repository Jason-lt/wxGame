...
//
// //如果文件里存了音量信息，则进行设置
// var mute = hall.GlobalFuncs.ReadBoolFromLocalStorage(scratch.SETTING_MUTE_KEY, false);
// if (mute) {
//     scratch.AudioHelper.setMusicVolume(0);
//     scratch.AudioHelper.setEffectsVolume(0);
// } else {
//     var music_vol = hall.GlobalFuncs.ReadNumFromLocalStorage(scratch.SETTING_MUSIC_KEY, 0.5); //如果没有存储，初始默认值0.5
//     scratch.AudioHelper.setMusicVolume(music_vol);
//     var effect_vol = hall.GlobalFuncs.ReadNumFromLocalStorage(scratch.SETTING_EFFECT_KEY, 1); //如果没有存储，初始默认值1
//     scratch.AudioHelper.setEffectsVolume(effect_vol);
// }
                    ...
                    wx.getSetting({
                        success:function(res) {
                            if (!res.authSetting['scope.userInfo']) {
                                ...
                                // var openData = scratch.GlobalFuncs.getOpenData();
                                // if (openData){
