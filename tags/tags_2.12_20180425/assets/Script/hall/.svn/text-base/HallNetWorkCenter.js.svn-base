...
hall.hallNetWorkCenter = {
    ...
    _onLocInfo: function (larr) {
        ...
        ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeUserFrom,[scene,query.from,query.appid,
            "CardStart",query.sourceCode,query.imageType,query.inviteCode,queryS]);
        if(query && query.sourceCode){
            ...
            //特殊场景处理
            if (query.ftID && ty.SystemInfo.ftID  == "" && !ddz.Share.isOnShare) {
                ty.SystemInfo.ftID = query.ftID;
                ddz.Share.shareKeywordReplace.inviteFriendID = query.ftID;
            }else {
                if (shareTicket && ty.SystemInfo.shareTicket == "" && !ddz.Share.isOnShare &&
                    query.sourceCode == ddz.Share.clickStatShareType.clickStatShareTypeRankList) {
