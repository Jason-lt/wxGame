...
var _is3311 = function(newarr, lz_obj, types) { //总共8个牌，只要找出2个3顺，剩余2牌一定符合规则
	...
	for (var index = 0; index < newarr.length - 4; index++) { //不包含2，从A开始找,出去大王，小王，2
		...
		if (left >= 0) { //只要癞子够凑成33就行，剩下的2个牌
			...
			if (need1 > 0) {
				n = ddz.GlobalFuncs.getLaiziNumByValue(tmp1.point);
				for (i = 0; i < need1; i++) {
			...
			if (need2 > 0) {
				n = ddz.GlobalFuncs.getLaiziNumByValue(tmp2.point);
				for (i = 0; i < need2; i++) {
			...
			var cards;
			if(cards2.count <= 3 && cards1.count <= 3){
				cards = cards2.concat(cards1);
			}else {
				cards = cards2.slice(0,3).concat(cards1.slice(0,3));
			}
			...
			for (i = newarr.length-1; i >= 0; i--) {
				if (i != index && i != index + 1) {
					...
					if(newarr[i].cards.length > 3){
						cards = cards.concat(newarr[i].cards.slice(3));
					}
				}
			}
...
var _is333111 = function(newarr, lz_obj, types) {
	...
	for (var index = 0; index < newarr.length - 5; index++) { //不包含2，从A开始找
		...
		if (left >= 0) { //只要癞子够凑成33就行，剩下的2个牌
			var cards;
			if(tmp3.count <= 3 && tmp2.count <= 3 && tmp1.count <= 3){
				cards = tmp3.cards.concat(tmp2.cards);
				cards = cards.concat(tmp1.cards);
			}else {
				cards = tmp3.cards.slice(0,3).concat(tmp2.cards.slice(0,3));
				cards = cards.concat(tmp1.cards.slice(0,3));
			}
...
var _is3333311111 = function(newarr, lz_obj, types) {
	...
	for (var index = 0; index < newarr.length - 6; index++) { //不包含2，从A开始找
		...
		if (left >= 0) { //只要癞子够凑成33就行，剩下的2个牌
			...
			var cards = tmp5.cards.concat(tmp4.cards);
			cards = cards.concat(tmp3.cards);
			cards = cards.concat(tmp2.cards);
			cards = cards.concat(tmp1.cards);
			...
			if (need3 > 0) {
				n = ddz.GlobalFuncs.getLaiziNumByValue(tmp3.point);
				for (i = 0; i < need3; i++) {
					cards.push(n);
				}
			}
			if (need4 > 0) {
				n = ddz.GlobalFuncs.getLaiziNumByValue(tmp4.point);
				for (i = 0; i < need4; i++) {
					cards.push(n);
				}
			}
			if (need5 > 0) {
				n = ddz.GlobalFuncs.getLaiziNumByValue(tmp5.point);
				for (i = 0; i < need5; i++) {
					cards.push(n);
				}
			}
