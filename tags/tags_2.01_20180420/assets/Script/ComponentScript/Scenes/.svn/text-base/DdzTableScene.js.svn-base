...
cc.Class({
    ...
    onDestroy:function () {
        ...
        if (this._rightChatController){
            this._rightChatController.removeChat();
            this._rightChatController = null;
        }
        if (this._leftChatController){
            this._leftChatController.removeChat();
            this._leftChatController = null;
        }
    ...
    clickChatButton:function(index,url,name) {
        ...
        if (index == ddz.LOCATION_SIGN.LEFT){
            ...
            if (this._leftChatController == null){
                cc.loader.loadRes('prefabs/userChat', function (err, prefab) {
                    ...
                    this._leftChatController.setArguments(_tableId, _roomId, this._mySeatIndex, to ,url, name);
            ...
            if (this._rightChatController == null){
                cc.loader.loadRes('prefabs/userChat', function (err, prefab) {
                    ...
                    this._rightChatController.setArguments(_tableId, _roomId, this._mySeatIndex, to ,url, name);
        ...
        // ddz.LOGD("", " 模拟 弹出对局流水,历史 ");
        //
        // ty.NotificationCenter.trigger(ddz.EventType.ACTION_DUIJU_LIUSHUI,{});   // 弹出对局流水,历史
    },
    ...
    getEmoPos: function(index){
        ...
        if (index == ddz.GlobalFuncs.GetNextIndex(this._mySeatIndex)) {
            ...
            // pos = this.head_regiht.getPosition();
            ...
            // pos = this.head_left.getPosition();
            ...
            if (result['toseat'] == ddz.GlobalFuncs.GetNextIndex(this._mySeatIndex)){   // 下家
                ...
                var _call_right = function() {
                    if (ddz.CHAT_COOLING_TIMER_RIGHT >= 0) {
                        // ty.NotificationCenter.trigger(ddz.EventType.UPDATE_COOLING_TIMER,ddz.CHAT_COOLING_TIMER_RIGHT);
                ...
                var _call_left = function() {
                    if (ddz.CHAT_COOLING_TIMER_LEFT >= 0) {
                        // ty.NotificationCenter.trigger(ddz.EventType.UPDATE_COOLING_TIMER,ddz.CHAT_COOLING_TIMER_LEFT);
    ...
    _startGameOverAnimation: function(result) {
        ...
        if (this._friendPanel){
            var score = this.tableInfo().ftInfo.getScore(this._mySeatIndex - 1);
            var delta = this.tableInfo().ftInfo.getDelta(this._mySeatIndex - 1);

            this._friendPanel.upDataScore('my', score, delta);

            score = this.tableInfo().ftInfo.getScore(leftindex - 1);
            delta = this.tableInfo().ftInfo.getDelta(leftindex - 1);

            this._friendPanel.upDataScore('left', score, delta);

            score = this.tableInfo().ftInfo.getScore(rightindex - 1);
            delta = this.tableInfo().ftInfo.getDelta(rightindex - 1);

            this._friendPanel.upDataScore('right', score, delta);
        }
