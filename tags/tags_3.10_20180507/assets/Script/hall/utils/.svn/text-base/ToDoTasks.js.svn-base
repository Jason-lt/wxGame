	...
	runPopInforWnd: function(params) {
		...
		if (desc) {
			...
			if (arr[0] == "fuhaoBuyFailed") {
				ty.NotificationCenter.trigger(ddz.EventType.DIAMOND_INSUFFICIENT,arr[1]);
				return
			}
		}
			...
			if (subaction == "quick_start"){	// 报名
				...
				this.onClickLeftButton = function(){
					...
					if (curScene.name ==  "TableScene") {
						...
						ddz.MsgFactory.getRoomLeave(tableinfo.roomId(), tableinfo.tableId(), tableScene._mySeatIndex, ddz.quickStartModel.getMixId());
					}
				};
				var str = "确定";
