...
ty.TuyooSDK = {
    ...
    */
    loginTuyooWithCode: function(code, userInfo) {
        ...
        // debugMode = hall.GlobalFuncs.ReadBoolFromLocalStorage(ddz.gameModel.DEBUG_MODE,false);
        if (debugMode){
        ...
        // };
        // hall.LOGE("==Login===","====tempData====="+JSON.stringify(tempData));
        hall.LOGE("==Login===","====tempData===== URL = "+JSON.stringify(sdkPath + 'open/v6/user/LoginBySnsIdNoVerify'));
        wx.request({
    ...
    rechargeOrder: function (params){
        ...
        wx.request({
            ...
            success: function(params) {
                hall.LOGD(null, 'tuyoo rechargeOrder success, params:' + JSON.stringify(params));
                // var checkData = params.data;
                // if (checkData.error && checkData.error.code == 1){
                ...
                if (results.code == 0) {
                    ...
                    hall.LOGE(null, 'tuyoo rechargeOrder success 创建订单成功, chargeData.mustcharge =' + chargeData.mustcharge);
                    if (chargeData && chargeData.mustcharge == 1) {
                        ...
                        wx.requestMidasPayment({
                            ...
                            success:function(params) {
                                // 支付成功
                                ty.Recharge.orderCallFunc(notifyUrl,platformOrderId);
                            },
                            fail:function(params) {    //{ errMsg, errCode }
                                hall.LOGE(null, '米大师支付 fail params = ' + JSON.stringify(params));
                                // 支付失败
...
wx.onShow(function (result) {
    ...
    if(query && query.sourceCode){//从小程序消息卡片中点入
        ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeUserFrom,[scene,query.inviteCode,ddz.Share.clickStatShareType[query.sourceCode],
            query.imageType,"CardActive",queryS]);
        //好友房打点
