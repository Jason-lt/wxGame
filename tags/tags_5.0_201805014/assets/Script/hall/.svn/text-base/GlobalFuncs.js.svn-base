...
hall.GlobalFuncs.gotoDdzTable = function(){
	...
	// 	hall.LOGE('global', '下一帧执行 跳转TableScene');
...
hall.GlobalFuncs.pushScene = function(sceneName,onLaunched){
	...
	// hall.LOGE("","file = [GlobalFuncs] fun = [pushScene] hall.SceneGather = " + JSON.stringify(hall.SceneGather));
	cc.director.loadScene(sceneName,onLaunched);
};
...
hall.GlobalFuncs.popScene = function(){
	...
	if (hall.SceneGather.length > 0) {
		...
		// hall.LOGE("","file = [GlobalFunc] fun = [popScene] hall.SceneGather = " + JSON.stringify(hall.SceneGather));
		// cc.director.loadScene(sceneName,onLaunched);
...
hall.GlobalFuncs.showDiamondWindowWithType = function (type) {
	...
	if (type == "new") {
		sceneName = 'ddz_diamond_friend';
	}else {
		sceneName = 'ddz_diamond';
	}

	var curScene = cc.director.getScene();
	if(curScene.name == sceneName && curScene.children[0].getComponent('ddz_diamond').inviteType == type){
		return;
	}
	var onLaunched = function () {
		var logicScene = cc.director.getScene();
...
hall.GlobalFuncs.onShowAwardTips = function (chip_tips) {
	if (chip_tips == 1) {
		cc.loader.loadRes('prefabs/award_tips', function (err, prefab) {
			var awardTips = cc.instantiate(prefab);
			cc.director.getScene().addChild(awardTips);
			ddz.GlobalFuncs.setToCenter(awardTips);
			var ani = awardTips.getComponent(cc.Animation);
			var clipName = ani.getClips()[0].name;
			var anim = ani.getAnimationState(clipName);
			anim.once("finished", function () {
			});
