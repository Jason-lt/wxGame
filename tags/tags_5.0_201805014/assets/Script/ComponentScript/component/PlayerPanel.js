...
cc.Class({
    ...
    onChatButton: function() {
        ...
        if (this._userInfo) {
            ...
            var name = this._userInfo.udataInfo.m_name;
            this._playController.clickChatButton(this._index,url,name);
        }
    },
    ...
    /**
     * 转换动画形像
     * @param actionName 要播放的动作名
     */
    refreshAvatarFore: function(actionName) {
        ...
        actionName = actionName || ddz.ROLE_ACTION.CHANGTAI;

        var spDataPath = hall.GlobalFuncs.getSpDataPath(isDiZhu, isMale);
        var isLeft = this._bLeft;
        //显示为地主或农民的动画形像
        ...
        cc.loader.loadRes(spDataPath, sp.SkeletonData, function (err, skeletonData) {
            this.spRole.skeletonData = skeletonData;
            this.spRole.setAnimation(0, actionName, true);
            var scX = 0.55;
            var scY = 0.55;
            ...
            if (isLeft){
                ...
                if (spDataPath == "spine/ddz_newoneself_dizhupo"){
                    //地主婆用小比例
                    scX = 0.51;
                    scY = 0.51;
                }
            ...
            this.spRole.node.scaleX = scX;
            this.spRole.node.scaleY = scY;
        }.bind(this));
    ...
    refreshAvatarForeFriend : function (origin) {
        ...
        // var myAvatarM = this.avatar.getComponent("Avatar");
        if(isDiZhu){
            var to_pos =  this._playController.getEmoPos(myIndex);
            if(origin){
                this._playController._friendPanel.showDizhuHatOrigin(to_pos);
            }else {
                this._playController._friendPanel.showDizhuHat(to_pos);
            }
            // myAvatarM.showDizhuHat(to_pos);
        }
        // else {
        //     myAvatarM.hideDizhuHat();
        // }
     ...
     */
    playRoleAction:function (actionName) {
        this.spRole.setAnimation(0, actionName, true);
    },
    ...
    showTimer:function (valTime) {
        ...
        this.playRoleAction(ddz.ROLE_ACTION.SIKAO);
    ...
    hideTimer:function () {
        this.unscheduleAllCallbacks();
        this.waitTimer.active = false;
        this.playRoleAction(ddz.ROLE_ACTION.CHANGTAI);
    },
