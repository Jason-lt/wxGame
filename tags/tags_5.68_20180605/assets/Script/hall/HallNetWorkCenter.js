...
hall.hallNetWorkCenter = {
    ...
    _onLocInfo: function (larr) {
        ...
        if (ty.SystemInfo.treasureID != "" && query.inviteCode) {
            hall.LOGW("","file = [HallNetWorkCenter] fun = [_onLocInfo] query.inviteCode = " + query.inviteCode);
            hall.LOGW("","file = [HallNetWorkCenter] fun = [_onLocInfo] ty.SystemInfo.treasureID = " + ty.SystemInfo.treasureID);
            var _boxUserId = query.inviteCode;
            ddz.gameModel.getBoxReward(parseInt(_boxUserId),ty.SystemInfo.treasureID);
            ty.SystemInfo.shareTicket = "";
        }
