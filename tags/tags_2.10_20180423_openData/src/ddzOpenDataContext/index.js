...
var updateRank = function(data){
	wx.setUserCloudStorage({
		...
		success : function () {
			logD("setUserCloudStorage===="+data.sumReward+"===="+ JSON.stringify(arguments));
		},
		fail : function () {
...
var showGroupRank = function(data){
	...
	wx.getGroupCloudStorage({
		...
		keyList:["avatarUrl","nickName","winnerCount",'userId','week','sumReward'],
...
var drawRankList = function (rankDatas,userId,week) {
	...
	while (j < rankDatas.length){
		...
		var k = j -1;
		while ( k >= 0 ){
			var number2 = rankDatas[k].sumReward;
			if(number1 > number2){
	...
	// logD("drawRankListDtatatataagghh+++++++++++++++++"+JSON.stringify(rankDatas));
	...
	if(rankDatas && rankDatas.length > 0){
		for (var i = 0 ; i < 21 ; i ++){
			...
			if (i < rankDatas.length){
				...
				if(rankDic.sumReward){
					context.fillText(rankDic.sumReward+"",480,baseHeight);
				}else {
					context.fillText("0",480,baseHeight);
				}
			}else {
				context.textAlign="center";
				context.fillText(rankS,40,baseHeight);
