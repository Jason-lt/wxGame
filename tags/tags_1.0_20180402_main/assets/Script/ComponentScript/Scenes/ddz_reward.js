...
cc.Class({
    ...
    onRewardButtonForRedIcon : function () {
        var tipsW = cc.instantiate(this.tipsWin);
    ...
    onDrawButton:function () {
        ...
        if(!draw.lossMoneyString || draw.lossMoneyString == 0){
            hall.MsgBoxManager.showToast("余额不足");
            return;
        }
        ...
        ddz.matchModel.getCashReward(parseFloat(draw.drawMoeyString)*100);
    ...
    onCashResult : function (argument) {
        ...
        if(!result.code || result.code != -1){
            ...
            window.setRedPacketNumber(parseInt(result.value)/100 +"");
            this.withDraw.active = false;
            this.redPacket.active = true;
            this.nowType = 4;
        }else {
            hall.MsgBoxManager.showToast(result.info);
        }
    },
