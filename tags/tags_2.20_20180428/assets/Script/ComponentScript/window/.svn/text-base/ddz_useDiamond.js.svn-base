...
cc.Class({
    ...
    setDetailWithDiamondCount:function(type,state,guan){
        ...
        if (this.sceneType == 1){
            ...
            buttonA = [{title :titlS, left :'ddz_main_icon_begin_v2', bottomType :0}];
            this.titleL.node.active = true;
            this.detailText.horizontalAlign = 0;
            this.detailText.string =
                "<img src='ddz_white_circle' height=8 width=8/><color=#ffffff> 闯过七关可平分每天奖励</color><br/>" +
                "<img src='ddz_white_circle' height=8 width=8/><color=#fffffff> 闯关失败可使用 <img src='dda_button_diamond' height=34 width=42/> 复活</color><br/>"+
                "<img src='ddz_white_circle' height= 8 width=8 /><color=#ffffff> 邀请朋友每天第一次登录 </color>" +
                "<img src='dda_button_diamond' height=34 width=42/><size='36' color=#fffffff> +1</color>";
        }else if(this.sceneType == 2){
            isHasCount = true;
            buttonA = [{title :"邀请得", right :"dda_button_diamond", bottomType :1}];
            this.detailText.horizontalAlign = 0;
            this.detailText.string = "<img src='ddz_white_circle' height=8 width=8/><color=#ffffff> 每天首次登录 </color>" +
                "<img src='dda_button_diamond' height=34 width=42/><size=36 color=#fffffff> +1</color><br/>"+
                "<img src='ddz_white_circle'  height=8 width=8/><color=#ffffff> 邀请朋友每天第一次登录 </color>" +
                "<img src='dda_button_diamond' height=34 width=42/><size=36 color=#fffffff> +1</color>";
        }
        if(isHasCount){
            this.coutLabel.node.active = true;
            this.coutLabel.string = "<color=#ffffff>已有：</c><img src='dda_button_diamond' height=34 width=42/><color=#ffffff> x"+this.diamondCount+"</color>";
        }else {
    ...
    setActiveForFalse : function () {
        ...
        if(this.parentScene){
            this.parentScene.setActiveForFalse();
        }
    },

    setActiveForFalseBack : function () {
        this.buttons.targetOff();
    },
    ...
    //         this.setActiveForFalse();
    //     }else{
    //         ddz.Share.shareWithType(ty.UserInfo.clickStatShareType.clickStatShareTypeGetDiamondHall);
    //     }
    // }
});
