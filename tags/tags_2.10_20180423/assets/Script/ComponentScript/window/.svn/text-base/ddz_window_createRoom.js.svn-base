...
cc.Class({
    ...
    getFriendConfigAction : function (value) {
        ...
        var window,window2,window3;
        window = this.createItem1.getComponent("ddz_item_createRoom");
        window.setRoundCount(rounds[0].nRound);
        window2 = this.createItem2.getComponent("ddz_item_createRoom");
        window2.setRoundCount(rounds[1].nRound);
        window3 = this.createItem3.getComponent("ddz_item_createRoom");
        window3.setRoundCount(rounds[2].nRound);

        var round = hall.GlobalFuncs.ReadNumFromLocalStorage(ddz.friendModel.CREATE_ROUND_COUNT, 0);
        if (round == rounds[0].nRound){
            window.setSelectedState(true);
            window2.setSelectedState(false);
            window3.setSelectedState(false);
        }else if(round == rounds[1].nRound){
            window.setSelectedState(false);
            window2.setSelectedState(true);
            window3.setSelectedState(false);
        }else if(round == rounds[2].nRound){
            window.setSelectedState(false);
            window2.setSelectedState(false);
            window3.setSelectedState(true);
        }else {
