...
cc.Class({
    ...
    onShareButton : function (event) {
        ...
        var shareType = ddz.Share.clickStatShareType.clickStatShareTypeWithDraw;
        ddz.Share.shareWithType(shareType);
    },

    getDiamond:function(){
        hall.LOGE("=="," file = [ddz_getRedPacket] fun = [getDiamond] ");
        ddz.GlobalFuncs.playZuanShi(true);
    },

    playAnimationAfterShareWithType : function (shareType) {

        if (shareType && shareType != ddz.Share.clickStatShareType.clickStatShareTypeWithDraw) {
            return;
        }
    ...
    updateButtonText:function(diamond,pointId){
        ...
        if (!diamond || diamond <= 0) {
            ...
            this.buttonText.string = "<color=#FFFFFF>分享到群</c><img src='dda_button_diamond'/><color=#FFFFFF>+1</c>";
        }
    },
