    ...
    if(query && query.sourceCode){//从小程序消息卡片中点入
       ...
       //特殊场景处理
        if (query && query.ftID) {
            ...
            //进入群排行榜
            if(query.sourceCode == ddz.Share.clickStatShareType.clickStatShareTypeRankList){//群排行榜
        ...
        //其他来源的打点
        var queryList = ["scene="+scene,"appid","from",queryS,ty.SystemInfo.gameId];
        for (var key in query){
            if(key == 'appid'){
                queryList[1] = query.appid;
            }
            if(key.indexOf('from') > -1){
                queryList[2] = query[key];
            }
        }
        ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeClickFromOther, queryList);
    }
