...
ddz._TableState = cc.Class({
    ...
    setLaizi: function(lz_point) {
        ...
        var lz_value = ddz.GlobalFuncs.GetCardValueByPoint(lz_point + 1);
        this.m_laiziNumber = ddz.GlobalFuncs.getLaiziNumByValue(lz_value);
    },
    ...
    parseTableState: function(result) {
        ...
        // 设置癞子牌点数
        if (typeof(result["wildcard"]) != 'undefined') {
            this.setLaizi(result["wildcard"]);
        }
        //三张底牌，长度为3     //用不上，暂时不管
        if (typeof(result["basecard"]) != 'undefined') {
            this.objectInfo.m_baseCard = result["basecard"];
        }
        //上把所出的牌
        if (typeof(result["topcard"]) != 'undefined') {
