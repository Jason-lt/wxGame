...
cc.Class({
    ...
    setDiamondCount:function (guangQiaNum) {
        ...
        this.numberlabel.string ="<color=#ffffff>已有 </c><img src='dda_button_diamond' height=34 width=42 /><size='36' color='#ffffff'> x"+countNunber+" </color>";
    ...
    onLoad:function () {
        ...
        var count = ddz.matchModel.getDiamondCountNeeded();
        this.noticelabel.string = "<color=#ffffff>使用 </c><img src='dda_button_diamond'  height=34 width=42/><color=#ffffff> x "+count+" 继续挑战本关</color>";

        var curWait = ddz.matchModel.getCurWaitInfo();
        if(curWait){
            var newUserProtect = curWait.newUserProtect;
            if(newUserProtect == -1){
                if (ddz.GlobalFuncs.checkFirstFail() && ddz.matchModel.getStageIndex() > 1){
                    ddz.GlobalFuncs.showRevialWindow(false,1);  //弹出非新手首次失败复活分享窗口
                }
            }else if(newUserProtect > 0){
                var curDate = hall.GlobalTimer.getCurDay();
                hall.GlobalFuncs.setInLocalStorage(ddz.matchModel.LAST_FAIL_DATE, curDate);
                if(ddz.matchModel.getStageIndex() < 6){
                    ddz.GlobalFuncs.showRevialWindow(false,2);  //弹出新手三次复活机会分享窗口
                }
            }
        }
    },
