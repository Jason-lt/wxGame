...
cc.Class({
    ...
    setShareImageType : function (type,data) {
        ...
        // this.changeBjSprite(this.backImageUrl);
        ...
        if(type == "invite"){
            ...
            var number = type == "showy_hughTotal" ? data.chips : type == "showy_winningStreak" ?  data.counts : data.multi;
            ...
            //
            // var number = type == "showy_hughTotal"? 9625000 : 1024;
    ...
    setTitleImage : function (number,pers) {
        ...
        image.onload = function (event) {
            ...
            var word = that.shareType == "showy_hughTotal" ? "总奖金"+hall.GlobalFuncs.formatGold(number) :
                that.shareType == "showy_winningStreak" ? "连胜" : "倍";
            var stokeWord = that.shareType == "showy_hughTotal" ? word : "打出"+ number+word;
            // context.fillText(stokeWord,320,88+that.exHeight+55);
            context.fillText(stokeWord,320,88+55);

            context.font = "50px Arial";
            // context.fillStyle = "#FFFFFF";
            // context.textAlign = "center";
            // context.fillText("超越全国"+pers+"%玩家",320,173+that.exHeight+55);
            context.fillText("超越全国"+pers+"%玩家",320,173+55);
        ...
        image.onload = function (event) {
            ...
            var cardCallBack = function () {
                ...
                if(cardNow < cardNumbers){
                    ...
                    var toX = cardNow > breakLine ? originX+gapX*(cardNow-breakLine) : originX+gapX*cardNow;
    ...
    saveToPhoneAddQRAndAvatar : function (shareBackImageUrl) {
        ...
        image.onload = function (event) {
            ...
            //添加白色底图
            var tempBackUrl = wx.env.USER_DATA_PATH +  "/res/raw-assets/resources/table/shareBackImage/shareTo_moments_bottom.jpg";
            var seImage = wx.createImage();
            seImage.src = tempBackUrl;
            seImage.onload = function (event) {
                var img = event.target;
                context.drawImage(img,0,946+that.exHeight*2,640,190);
                        ...
                        fs.writeFile({
                            ...
                            success : function () {
                                ...
                                qrImage.onload = function (event) {
                                    var img = event.target;
                                    context.drawImage(img,438,956+that.exHeight*2,144,144);
                                    var r = 31;
                                    context.beginPath();
                                    context.arc(510,1028+that.exHeight*2,r,0,2*Math.PI);
                                    context.stroke();
                                    context.clip();
                                    var avatarPic = wx.createImage();
                                    avatarPic.onload = function (event) {
                                        var img = event.target;
                                        context.drawImage(img,510-r,1028-r+that.exHeight*2,r*2,r*2);
                                        var tempFilePath = tempCanvas.toTempFilePathSync({
                                            x: 0,
                                            y: 0,
                                            width: 640,
        ...
        var saveImageToPhone = function () {
            wx.saveImageToPhotosAlbum({
                ...
                success:function(res) {
                    ddz.GlobalFuncs.showNormalTipsWindow("图片已保存到手机相册，快去朋友圈分享吧",[{title:"确定",callFunc:function () {
                        if(that.shareType == "invite"){
                            that.node.destroy();
                        }
                    }}]);
