...
ddz.Share = {
    ...
    getShareTicketInformation : function (result) {
        ...
        hall.LOGW("","file = [Share] fun = [getShareTicketInformation] reultString = "+JSON.stringify(reultString));
        ...
        if(toDay != lastShareTime){
            ...
            var shareTicketsDic = JSON.parse(shareTickets);
    ...
    playAnimationAfterShareWithType : function () {
        ...
        //分享点什么都不验证,直接发钻石
        if(ddz.Share.unSensitivePersonage == 1){
            this.endWithSuccessShare();
            return;
        }
        ...
        //分享点只验证群,直接发钻石
        if(ddz.Share.unSensitivePersonage == 2){
            this.endWithSuccessShare();
            return;
        }
        //要验证群的处理
