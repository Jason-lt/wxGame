...
cc.Class({
    ...
    onActionChat: function(result) {
        ...
        cc.loader.loadRes(gStr, function (err, prefab) {
            ...
            if (Math.abs(to_pos.x - from_pos.x) < 100){
                ...
                midx = to_pos.y - 300;
                midy = from_pos.x + (to_pos.x - from_pos.x)/3;
            }else {
                midx = -200;
    ...
    onActionGameWin: function(result) {
        // 首先判断是否是流局
        var that = this;
        if (result && result.hasOwnProperty('nowin') && result['nowin'] === 1) { //流局重新发牌
        ...
        ddz.matchModel.isDiZhuWin = result.dizhuwin;
    ...
    _startGameOverAnimation: function(result) {
        ...
        // ty.NotificationCenter.trigger(ddz.EventType.UPDATE_REMOVE_CHAT);

        //判断游戏结果
        if (this.isErdou()) {
            this._topPlayerController.setPrepareVisible(false);
            this._topPlayerController.setChatButtonActive(false);
    ...
    _getGameResult: function(result) {
        ...
        if (result && result.hasOwnProperty('nowin') && 1 === result['nowin']) { //暂时不处理流局
            // 流局
            return 'nowin';
        }
    ...
    _getIndexFromPos: function(pos) {
        ...
        // var firstLineIndex = Math.floor((pos.x - this.firstLineOriginX) / this._cardsIntervalX);
        // var secondLineIndex = Math.floor((pos.x - this.secondLineOriginX) / this._cardsIntervalX);
        //
