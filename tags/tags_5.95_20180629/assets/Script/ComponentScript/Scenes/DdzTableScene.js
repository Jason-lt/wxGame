...
cc.Class({
    ...
    _initUI: function() { //先创建自身的UI，再创建子容器的UI，因为原来代码土，子容器用到了父容器内容，不先创建自身的，会出错
        ...
        if (this.tableInfo().getSceneType() == ddz.Enums.SceneType.MATCH){
            ...
            if (bc){
                if (bc.start && winnerCount > bc.start) {
                    isTop = false;
                    this.showMatchBanner(bc,isTop);
                }
            }else if (btc){
                if (btc.start && winnerCount > btc.start) {
                    isTop = true;
    ...
    onActionCardNote:function(result){
        if (result && result.isOpen == 1) {
            ...
            if (ddz.isClickJiPaiQi){
                hall.MsgBoxManager.showToast({"title":"在牌局结束时看广告可得记牌器哦~"});
            }
        }
        ddz.isClickJiPaiQi = false;
    },
    ...
    onActionGameWin: function(result) {
        ...
        //延时弹出宝箱
        if (result["box_share"] && result["box_share"].boxId &&
            result["box_share"].boxId != ""){
            ddz.Share.shareKeywordReplace.inviteTreasureID = result["box_share"].boxId;
