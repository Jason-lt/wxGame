    ...
    changeContentLabelString : function (guanS) {
        ...
        if(desInfo && desInfo.detailStages){
            ...
            if(stageInfo && stageInfo.length > guan){
                ...
                var count = des.count;
                if(count){
                    this.contentLabel.string = "<color=#1A6951>使用 </color><img src='dda_button_diamond_black'/>" +
                        "<color=#1A6951> x"+ count+"继续挑战第"+guanS+"关</color>";
                    return;
                }
            }
        }
        this.contentLabel.string = "<color=#1A6951>使用 </color><img src='dda_button_diamond_black'/><color=#1A6951> x1继续挑战第"+guanS+"关</color>";
    },
