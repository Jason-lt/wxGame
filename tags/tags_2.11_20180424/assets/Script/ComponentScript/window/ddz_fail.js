...
cc.Class({
    ...
    setDiamondCount:function (guangQiaNum) {
        ...
        this.numberlabel.string ="<color=#ffffff>已有 </c><img src='dda_button_diamond' /><size='36' color='#ffffff'> +"+countNunber+" </color>";
    ...
    onLoad:function () {
        ...
        var count = ddz.matchModel.getDiamondCountNeeded();
        this.noticelabel.string = "<color=#ffffff>使用 </c><img src='dda_button_diamond' /><color=#ffffff> + "+count+" 继续挑战本关</color>";

        if (ddz.GlobalFuncs.checkFirstFail() && ddz.matchModel.getStageIndex() > 1){
            //弹出分享窗口
            ddz.GlobalFuncs.showRevialWindow();
        }
    },
