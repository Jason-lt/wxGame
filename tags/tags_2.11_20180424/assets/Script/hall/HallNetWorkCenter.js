    ...
    _onLocInfo: function (larr) {
        ...
        if(query && query.sourceCode){
            ...
            ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeClickFromCardStart,
                [query.sourceCode,query.imageType,query.inviteCode,queryS,ty.SystemInfo.gameId]);
            if(query.sourceCode == ddz.Share.clickStatShareType.clickStatShareTypeInviteFriend){
            ...
            //特殊场景处理
            if (query.ftID && ty.SystemInfo.ftID  == "") {
                ty.SystemInfo.ftID = query.ftID;
                ddz.Share.shareKeywordReplace.inviteFriendID = query.ftID;
            }else {
                if (shareTicket && ty.SystemInfo.shareTicket == "" && !ddz.Share.isOnShare && query.sourceCode == ddz.Share.clickStatShareType.clickStatShareTypeRankList) {
            ...
            if (curDay != lastDay && query.inviteName && query.sourceCode != ddz.Share.clickStatShareType.clickStatShareTypeInviteFriend) {
                hall.GlobalFuncs.setInLocalStorage(ddz.Share.LAST_LOGIN_TIME,curDay);
                var string =
                    "<color=#1A6951>成功帮助 "+query.inviteName+
                    " 获得</color><img src='dda_button_diamond_black' height=38 width=44/><color=#1A6951> +1<c/><br/><color=#1A6951>每期仅能帮助一位好友哦</c>";
                ddz.GlobalFuncs.showTipsWindowWithString(string,null,"确定");
            }
        }else {
            //其他来源
        ...
        if (ddz.waitGetRevial && ddz.waitGetRevial.type == 'showWindow'){
            ...
            ddz.GlobalFuncs.showRevialWindow(true);
        ...
        // }
        hall.LOGE(null, "+++++++++shareTicket++++++++"+ty.SystemInfo.shareTicket);
        if (roomid != 0 && tableid != 0) {
            ...
            if(ty.SystemInfo.shareTicket != "" ){
                ...
                hall.LOGE(null, "file = [HallNetWorkCenter] fun = [_onLocInfo] 1111");
                if (!ddz.matchResultPanel){
                    hall.LOGE(null, "file = [HallNetWorkCenter] fun = [_onLocInfo] 2222");
