...
cc.Class({
    ...
    onLoad: function () {
        ...
        var checkLogin = function () {
            if (!ty.UserInfo.userId){
                ...
                if (debugMode){
                    ty.TuyooSDK.getSystemType(function (result) {
                        if (result.brand == "devtools"){
                            ty.TuyooSDK.wechatLoginDebug()
                        }
                        else{
                            wx.getSetting({
    ...
    onEnterFriend : function () {
        ddz.matchModel.showPopWinByPreFab('prefabs/ddz_window_createRoom', function (preFabNode) {
