...
ddz.Share = {
    ...
    shareImformation: function shareImformation(type, shareCfg, titleString, imageUrl, query, sharePoint, successCallBackFun, failCallBackFun) {
        ...
        if (type == ddz.Share.onShareType.clickStatShareTypeInviteFriend) {
            ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeInviteFriendClick, [ddz.Share.shareKeywordReplace.inviteFriendID]);
        }
        ...
        wx.shareAppMessage({
            ...
            success: function success(result) {
                ...
                if (type == ddz.Share.onShareType.clickStatShareTypeInviteFriend) {
                    ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeInviteFriendSuccess, [ddz.Share.shareKeywordReplace.inviteFriendID]);
                }
                // 免费宝箱
                if (type == ddz.Share.onShareType.clickStatShareTypeGiveProp) {
                    ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeGiveProp, [ddz.Share.shareKeywordReplace.inviteTreasureID]);
                }
    ...
    playAnimationAfterShareWithType: function playAnimationAfterShareWithType() {
        ...
        //不是分享点,不做任何处理
        if ((!ddz.Share.sharePoint || ddz.Share.sharePoint == 0) && ddz.Share.shareType != ddz.Share.onShareType.clickStatShareTypeRevial) {
            ddz.Share.shareType = "";
            ddz.Share.resultType = 0;
            return;
        }
