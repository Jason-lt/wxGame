...
ddz.gameModel = {
    ...
    getNewRedBagReward: function getNewRedBagReward() {
        if (ddz.gameModel.registerDays == null) {
            return;
        }
        var pars = {
            "cmd": "game",
     ...
     * 获取分享点分享方式
     */
    getShareVideoConfig: function getShareVideoConfig() {
        var params = {
            'cmd': 'game',
            'params': {
                'action': 'common_config',
                'gameId': ty.SystemInfo.gameId,
                'configKey': 'shareVideoConfig'
            }
        };
        hall.MsgFactory._sendCmd(params);
    },
    ...
    parseGame: function parseGame(value) {
        ...
        switch (value.result.action) {
            ...
            case "new_coupon_reward":
                {
                    //新人红包,回归红包 弹窗
                    if (value.result && value.result.popup == 1) {
                        if (value.result.registerDays == 0) {
                            hall.GlobalFuncs.giftRedBag(1);
                        } else if (value.result.registerDays > 0) {
                            hall.GlobalFuncs.giftRedBag(2);
                        }
            ...
            case "gain_new_coupon":
                {
                    //新人红包,回归红包 奖励
                    if (value.result && value.result.success == 1) {
                        if (value.result.rewards && value.result.rewards[0]) {
                            if (value.result.rewards[0].itemId == "user:coupon") {
    ...
    parseCommonConfig: function parseCommonConfig(value) {
        ...
        switch (value.configKey) {
            ...
            case "shareVideoConfig":
                {
                    this.saveShareVideoConfigJson(value.config);
                    break;
                }
        }
    },
    ...
    saveShareVideoConfigJson: function saveShareVideoConfigJson(value) {
        if (hall.GlobalFuncs.isEmptyObject(value)) {
            return;
        }
        this.shareVideoConfig = value;
        ddz.gameConfig.shareVideoConfig = value;
    },
    getShareVideoConfigJson: function getShareVideoConfigJson() {
