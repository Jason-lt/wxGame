...
cc._RF.push(module, '03a45ujieVNMaIjVCnJAKQe', 'DdzTableScene', __filename);
// Script/ComponentScript/Scenes/DdzTableScene.js
...
cc.Class({
    ...
    //初始游戏
    initPlayGame: function initPlayGame() {
    ...
    //点击金币商城
    onClickCoinStore: function onClickCoinStore() {
    ...
    onClickReStart: function onClickReStart() {
        ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeButtonClick, ["clickReStart", "click"]);
    ...
    _initUI: function _initUI() {
        //先创建自身的UI，再创建子容器的UI，因为原来代码土，子容器用到了父容器内容，不先创建自身的，会出错
    ...
    _PlayInSingle: function _PlayInSingle() {
        //单机
    ...
    _singleJiaodz: function _singleJiaodz(target, bLeft) {
        ...
        if (bLeft) {
            ...
            if (this._tableInfo.config.m_grab == 1) {
                //抢地主
                if (this._mySeatinfo.model.m_call == 1) {
                    //如果玩家叫了地主，则必须再次让玩家抢地主
            ...
            if (this._tableInfo.config.m_grab == 0 && call == 3) {
                //抢地主
                this.node.runAction(cc.sequence(cc.delayTime(ddz.AIRobots.intervalTime), cc.callFunc(this._finishJiaodz.bind(this), this)));
            } else {
                //叫地主
    ...
    findActivePlayCards: function findActivePlayCards() {
    ...
    //初始化三个座位的牌
    _initCardsInSingle: function _initCardsInSingle() {
        // 上报单机游戏开始时间、银币数
    ...
    _setSingleCardsInfo: function _setSingleCardsInfo() {
    ...
    playAnimationSoundByCardType: function playAnimationSoundByCardType(type, bMale, userPosition) {
        // 由于产品要求，在播放连对和顺子动画时需要根据出牌人的位置来播放动画
        var sound;
        var special_effect;
        if (!type) {
            return;
    ...
    onActionCall: function onActionCall(result) {
        ...
        if (this._tableInfo.config.m_grab == 1 && this._mDizhuCalled == false && call == 1) {
            //如果是抢地主模式，第一个叫1分，则地主被叫，接下来就是抢地主或者不抢
            this._mDizhuCalled = true;
        }
    ...
    _startGameOverAnimation: function _startGameOverAnimation(target, result) {
    ...
    //游戏结束后显示剩下的牌
    _showCardsWhenGameOver: function _showCardsWhenGameOver(leftindex, rightindex) {
        //明牌标示图片
        if (this._mySeatinfo) {
                    ...
                    if (2 == pc[m].count) {
                        ...
                        for (i = this._cards.length - 1; i >= 0; i--) {
                            if (this._cards[i]._info._point == nPoint && this._cards[i]._info._number != nCount) {
                                if (active && this._isInShunzi(i)) {} else {
                                    this._touchedCards.push(this._cards[i]);
                                }
                            }
                        }
...
cc._RF.pop();
        ...
        //# sourceMappingURL=DdzTableScene.js.map
