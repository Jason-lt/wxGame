(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Script/hall/GlobalFuncs.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '6a2c3O/mspOpLP03XhBLc8E', 'GlobalFuncs', __filename);
// Script/hall/GlobalFuncs.js
...
hall.GlobalFuncs.FindInArrayBuFun = function (array, func) {
	//array.find是ECMA2015才提供，可能在一些设备上不支持
...
hall.GlobalFuncs.ChangeSpriteTexure = function (sprite, texureName) {
	//用setSpriteFrame一定会自动适应大小，如果需要强制不改变大小，需要设置scale，这会导致变形，所以去掉bAdjustSize参数
	var frme = cc.spriteFrameCache.getSpriteFrame(texureName);
	if (frme) sprite.setSpriteFrame(frme);
};

hall.GlobalFuncs.ChangeSpriteTexureWithSize = function (sprite, texureName, size) {
	//强制拉伸缩放frame自动适应大小，可能会导致扭曲，为了尽量少改动代码，不在上面方法增加参数，而是新建一个方法
	var frme = cc.spriteFrameCache.getSpriteFrame(texureName);
...
hall.GlobalFuncs.formatSeconds = function (value) {
	...
	var result = "";
	if (theTime > 9) result = "" + parseInt(theTime);else result = "0" + parseInt(theTime);
	if (theTime1 >= 0) {
		if (theTime1 > 9) result = "" + parseInt(theTime1) + ":" + result;else result = "0" + parseInt(theTime1) + ":" + result;
	}
	if (theTime2 >= 0) {
		if (theTime2 > 9) result = "" + parseInt(theTime2) + ":" + result;else result = "0" + parseInt(theTime2) + ":" + result;
	}
	return result;
};
...
hall.GlobalFuncs.giftRedBag = function (state) {
	...
	ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeShowWindow, ["ddz_new_reweard", state]); //state == 1 新人红包;  state == 2 回归红包
	if (ddz.newReward) {
...
hall.GlobalFuncs.rewadAni = function (num) {
	ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeShowWindow, ["ddz_reward_ani"]); //state == 1 新人红包;  state == 2 回归红包
	if (ddz.rewardAni) {
...
ddz.GlobalFuncs.getWinStreak = function () {
	var key = 'WIN_STREAK_KEY';
	var cnt = hall.GlobalFuncs.ReadNumFromLocalStorage(key, 0);
	return cnt;
};

ddz.GlobalFuncs.setWinStreak = function (val) {
	var key = 'WIN_STREAK_KEY';
	hall.GlobalFuncs.setInLocalStorage(key, val);
};
