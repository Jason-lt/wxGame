...
cc._RF.push(module, '2dbd2fNCjpPFqIqAEel8u0S', 'ToDoTasks', __filename);
...
cc.Class({
	...
	runEverydayLogin: function runEverydayLogin(params) {
		...
		if (subclick && subclick["action"] == "pop_first_recharge") {
			//首冲礼包的todotask可能包含在这里传过来，也可能独立传过来，这里处理被包含传过来的特殊情况。//TODO:对todotask的处理需要优化。。。
		...
		if (sub) {
			...
			if (subaction == "pop_pay_order") {
				...
				hall.GlobalFuncs.showPopWinByPreFab(preFabPath, function (preFabNode) {
					...
					var testArray = [{ title: "取消" }, { title: _titile, left: "dda_button_diamond" }];
