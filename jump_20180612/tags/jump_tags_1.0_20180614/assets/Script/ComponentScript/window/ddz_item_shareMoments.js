...
cc.Class({
    ...
    setShareImageType : function (type,data) {
        ...
        if(type == "invite"){
            ...
            jump.Share.shareKeywordReplace.goldWinReward = number;
            jump.Share.shareKeywordReplace.surpassPercent = data.percent;
        }
    },
        ...
        var saveImageToPhone = function () {
            wx.saveImageToPhotosAlbum({
                ...
                success:function(res) {
                    jump.GlobalFuncs.showNormalTipsWindow("图片已保存到手机相册\n快去朋友圈分享吧",[{title:"确定",callFunc:function () {
                        if(that.shareType == "invite"){
                            that.node.destroy();
                        }
                    }}]);
