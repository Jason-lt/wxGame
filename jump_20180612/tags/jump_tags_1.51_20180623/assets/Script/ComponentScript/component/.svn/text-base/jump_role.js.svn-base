...
cc.Class({
    ...
    playToolEffect : function (pedalProperty) {
        ...
        if(pedalProperty.isMonster == 1){
            jump.AudioHelper.playEffect(jump.EffectPath_mp3.jumpOnMonster,false);
            return;
        }
        if(pedalProperty.sectionWidth){
            if(-pedalProperty.sectionWidth > jump.gameModel.pedalGapWidth || jump.gameModel.pedalGapWidth > pedalProperty.sectionWidth){
                // if(pedalProperty.pedalType != "disablePedal" && pedalProperty.isMonster != 1) {
                //     jump.AudioHelper.playEffect(jump.EffectPath_mp3.jumpEffect,false);
                // }
                if(pedalProperty.treadCount != 0 && pedalProperty.isMonster != 1) {
                    jump.AudioHelper.playEffect(jump.EffectPath_mp3.jumpEffect,false);
                }
                return;
            }
        ...
        if(!pedalProperty.aniType || pedalProperty.aniType == "disable") {//正常弹跳起来的踏板,弹跳的声音
        // if(pedalProperty.pedalType == "normalPedal" || pedalProperty.pedalType == "oncePedal" || pedalProperty.pedalType == "movePedal") {
            jump.AudioHelper.playEffect(jump.EffectPath_mp3.jumpEffect,false);

        }else if(pedalProperty.pedalType.indexOf("disablePedal") > -1) {//不可踩踏板,碎裂的声音
            jump.AudioHelper.playEffect(jump.EffectPath_mp3.disableEffect,false);

        }else if(pedalProperty.pedalType.indexOf("springRing") > -1) {//弹簧类,播放弹簧声音,增加速度
        ...
        }else if(pedalProperty.pedalType.indexOf("trampoline") > -1){//蹦床类,播放蹦床声音,增加速度,播放翻滚动画
        ...
        }else if(pedalProperty.aniType == "backPack"){
        // }else if(pedalProperty.pedalType == "bambooDragonfly" || pedalProperty.pedalType == "rocketTube"){
            ...
            for (var i = 0 ; i < toolNameList.length ; i ++ ){
                if(pedalProperty.pedalType.indexOf(toolNameList[i]) > -1){
                    index = i;
                }
            }
            ...
            var effectPath;
            if(pedalProperty.pedalType.indexOf("bambooDragonfly") > -1){
    ...
    onLoad : function() {
        ...
        ty.NotificationCenter.listen(jump.EventType.SHOULD_GAME_OVER,this.shouldGameOverAction ,this);
    },
    shouldGameOverAction : function () {
