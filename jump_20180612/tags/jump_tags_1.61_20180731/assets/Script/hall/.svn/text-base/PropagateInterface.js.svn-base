...
ty.PropagateInterface = {
    ...
    getShareConfigInfo: function(reqObj) {
        ...
        var successcb = function(ret) {
            jump.gameModel.saveShareConfigFromHttp(ret.retmsg);
            ty.NotificationCenter.trigger(jump.EventType.GET_SHARE_CONFIG_SUCCESS, ret);
        };

        var failcb = function(ret) {
            ty.NotificationCenter.trigger(jump.EventType.GET_SHARE_CONFIG_FAIL, ret);
        };
    ...
    getUserFeatureInfo: function() {
        ...
        var finalUrl = ty.SystemInfo.shareManagerUrl + '?' + paramStrList.join('&');
        var successcb = function(ret) {
            ty.NotificationCenter.trigger(jump.EventType.GET_USER_FEATURE_SUCCESS, ret);
        };

        var failcb = function(ret) {
            ty.NotificationCenter.trigger(jump.EventType.GET_USER_FEATURE_FAIL, ret);
        };
        ty.HttpUtil.httpGet({'url':finalUrl}, successcb, failcb);
    },
