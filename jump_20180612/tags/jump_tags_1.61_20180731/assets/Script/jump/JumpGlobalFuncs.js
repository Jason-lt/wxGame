 ...
 * 展示下一个超越
 */
jump.GlobalFuncs.getTheNextSurpass = function () {
	var openDataContext = jump.GlobalFuncs.getOpenData();
	if(!openDataContext){
		return;
	}
	openDataContext.postMessage({
		method:'getNextSurpass',
		userId:ty.UserInfo.userId,
		rankType : "sumScore",
		sumScore : jump.gameModel.totalScore
	})
};
/**
 * 初始化
...
//展示复活面板
jump.GlobalFuncs.showResurgencePanel = function (resurgence) {
	cc.loader.loadRes('prefabs/jump_shareResurgence', function (err, prefab) {
		var preFabNode = cc.instantiate(prefab);
		cc.director.getScene().addChild(preFabNode);
		jump.GlobalFuncs.setToCenter(preFabNode);
		var com = preFabNode.getComponent('jump_shareResurgence');
		com.setResurgenceConfig(resurgence);
	}.bind(this));
};
//展示暂停面板
jump.GlobalFuncs.showPausePanel = function () {
	cc.loader.loadRes('prefabs/jump_pause', function (err, prefab) {
		var preFabNode = cc.instantiate(prefab);
		cc.director.getScene().addChild(preFabNode);
		jump.GlobalFuncs.setToCenter(preFabNode);
	}.bind(this));
