...
ty.TuyooSDK = {
    ...
    //                                 },
    //                                 fail:function () {
    //                                     ty.NotificationCenter.trigger(jump.EventType.START_AUTHORIZATION_FAILED);
    //                                     jump.needReLogin = true;
    ...
    wechatLoginDirect: function(loginCode) {
        ty.ServerStateManager.checkServerState(function () {
            ...
            //     };
            //
            //     ty.Timer.setTimer(cc.director, checkUserInfo, 5 / 60,1);
            // };

            if (loginCode){
                hall.LOGD(null, 'if (loginCode)');
    ...
    loginTuyooWithCode: function(code, userInfo, iv, encryptedData) {
        ...
        debugMode = hall.GlobalFuncs.ReadBoolFromLocalStorage(jump.gameModel.DEBUG_MODE,debugMode);
        if (debugMode){
        ...
        jump.Share.shareKeywordReplace.wechatName = userInfo.nickName;
    ...
    //     jump.GlobalFuncs.getFriendInfo();
    //     var checkUserInfo = function () {
    //         // var openDataContext = wx.getOpenDataContext();
    //         // var sharedCanvas = openDataContext.canvas;
    //         // var context = sharedCanvas.getContext("2d");
    ...
    //             ty.Timer.cancelTimer(cc.director, checkUserInfo);
    //             jump.gameModel.game_friendData = context['game_friendData'];
    //         }
    //     };
