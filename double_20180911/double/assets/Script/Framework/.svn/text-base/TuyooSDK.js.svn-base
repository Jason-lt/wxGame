...
wx.onShow(function (result) {
    ...
    if(query && query.sourceCode){ //从小程序消息卡片中点入
        if(query.sourceCode == double.Share.onShareType.grouprankshare && scene == 1044){ //群排行榜
    ...
    if(double.Share.isOnShare){
        double.Share.tcpCallback = true;
        var date = hall.GlobalFuncs.ReadStringFromLocalStorage(double.gameModel.ONHIDE_DATE);
        var date2 = new Date().getTime();
        var timeGap = date2-date;
        if(double.Share.shareRec){
            double.Share.shareTimeEnough = timeGap > double.GameWorld.generalConfig.shareRecTime;
        }else {
            double.Share.shareTimeEnough = true;
        }
...
wx.onHide(function () {
    ...
    hall.onHide = true;
    double.Share.shareTimeEnough = false;
    var date = new Date().getTime();
    hall.GlobalFuncs.setInLocalStorage(double.gameModel.ONHIDE_DATE, date);
    hall.LOGW('',"+++++++++++++++++onHide+++++++++++++++++");
    ty.TCP.close();
});
