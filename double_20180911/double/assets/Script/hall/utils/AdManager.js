...
hall.adManager = {
    ...
    videoAdId : "adunit-c92b38e70d4c7427",
    bannerAdId : "adunit-8c558c0bf3a346b6",
    ...
    showBannerAd:function (refresh) {
        ...
        if(!double.curBannerAd){
            double.curBannerAd = wx.createBannerAd({
                adUnitId: hall.adManager.bannerAdId,
                style: {
                    left:0,
                    top:top,
                    width: screenWidth
        ...
        //     }
        // });
        double.curBannerAd.onError(function (err) {
            hall.adManager.showBanner({x:cc.director.getWinSize().width/2,y:83});
            hall.LOGW('double.curBannerAd.onError:' + err);
        });
        ...
        wx.request({
            ...
            success: function (res) {
                ...
                if(res.data.loc && res.data.loc[2]){
                    ...
                    //TODO:TEST
                    // ty.UserInfo.isInBSGS = true; //是否在北上广深
                }
                hall.LOGE("===",'requestLocalIPInfo ==>' + JSON.stringify(res));
            },
            fail: function (res) {
                console.log('fail');
            }
