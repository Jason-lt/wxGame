...
double.sxAdmanager = {
    ...
    show: function (dx,dy) {//, scale) {
        try {
            ...
            if (this.adIcon) {
                ...
                this.adIcon.x = dx;
                this.adIcon.y = dy;
                hall.LOGE("show","=====show===="+this.adIcon.x+"======"+this.adIcon.y+"===="+this.adIcon.scale);
    ...
    loadConfigFile: function (callback) {
        try {
            ...
            // xhr.open("GET", "https://sanxqn.nalrer.cn/tysanxiao/box/adconfig/configDaoLiu11.json", true);
            xhr.open("GET", "https://nslyqn.nalrer.cn/nsly/crazygun/growth/adConfigFile/configDaoLiu_2.json", true);
            // https://nslyqn.nalrer.cn/nsly/crazygun/growth/adConfigFile/configDaoLiu.json
                ...
                if (xhr.readyState === 4) {
                    if ((xhr.status >= 200 && xhr.status < 300)) {
                        ...
                        try {
                            ...
                            // 游戏开关
                            self.sxAdInfo = ret;
                            double.sxAdmanager.refreshIconsWeight();
                            callback();
                    ...
                    if (this.sxAdInfo.icons[iconIndex].openType == 0) {
                        ...
                        wx.navigateToMiniProgram({
                            ...
                            success: function (res) {
                                double.sxAdmanager.saveItem(double.sxAdmanager.sxAdInfo.icons[iconIndex].openData[index].imgurl);
                                //刷新权重
                                double.sxAdmanager.refreshIconsWeight();
                                double.sxAdmanager.initWeight();
                                //刷新icon
                                double.sxAdmanager.switchIcon();
