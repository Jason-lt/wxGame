...
cc.Class({
    ...
    chooseLevelObject : function () {
        ...
        var nextNewLevel = 0;
        var addTemp = 2;
        while (nextNewLevel < double.GameWorld.totalLevel && double.GameWorld.totalLevel < 92){
    ...
    getNextObjectType : function () {
        ...
        //this.node.getChildByName('test').getComponent(cc.Label).string = double.GameWorld.objectType+':'+double.GameWorld.objectInitializedSpeed+':'+double.GameWorld.objectAccelerated+':'+double.GameWorld.objectHAccelerated;
    },
        ...
        if(double.GameWorld.levelObjectNumber <= 0){
            if(hall.GlobalFuncs.ReadBoolFromLocalStorage(double.gameModel.IS_NEW_USER,true)){
                ...
                animation.once("finished", function () {
                    ...
                    double.GameWorld.guiding = false;
                    hall.adManager.hideBannerAd();
                    double.GlobalFuncs.showTitlePage();
                });
                animation.play();
    ...
    gameOverShowWindow : function () {
        ...
        if(hall.GlobalFuncs.ReadBoolFromLocalStorage(double.gameModel.IS_NEW_USER,true)){
            ...
            this.doubleInfoNode.active = false;
            hall.adManager.showBannerAd();
            if(double.GameWorld.levelObjectNumber == double.GameWorld.levelObjectList.length || double.GlobalFuncs.getRewardOpenType("resurgence") == "omit"){
                double.GlobalFuncs.showGameOverResult();
            }else {
                double.GlobalFuncs.showResurgence();
            }
            this.surpassSprite.active = false;
        }
    },
    //从暂停页回到主页
    ...
    setSurpassNode : function () {
        ...
        this.surpassSprite.active = true;
    },
