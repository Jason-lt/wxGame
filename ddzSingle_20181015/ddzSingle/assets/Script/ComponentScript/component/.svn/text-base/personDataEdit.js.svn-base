...
cc.Class({
    ...
    setDefaultInfo:function(){
        switch (this.infoProperty){
            ...
            case "sex": //性别
                ...
                if (hall.ME.udataInfo.m_sex == ddz.Enums.PlayerSexEnum.SEX_MALE){ //男
    ...
    onCloseXiaLaView:function(){
        this.scrollView.node.active = false;
        this.selectBg.setScaleY(1);
    },
    

    setParentCom:function(_com){
        this.parentCom = _com;
    },

    setConDefault:function(){ // 星座
        if (this.xingzuoSet){
            return;
        }
        if (this.personInfo) {
    ...
    setProVinceDefault:function(){ // 省份
        if (this.shengfenSet) {
            return
        }
