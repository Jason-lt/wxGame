...
cc._RF.push(module, '2ffa4zWgiVDSrJvXpLtCt9X', 'TuyooSDK', __filename);
...
wx.onShow(function (result) {
    ...
    if (query && query.sourceCode) {
        ...
        if (query && query.ftID && !ddz.Share.isOnShare) {
            ...
            //进入群排行榜
            if (query.sourceCode == ddz.Share.onShareType.clickStatShareTypeRankList && !ddz.Share.isOnShare && scene == 1044) {
                //群排行榜
        ...
        ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeUserFrom, [scene, query.from, query.appid, "CardActive"]);
    }
