(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Script/ComponentScript/component/ddzsingle_robot.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '8964bazayhPrqbbnCcoumZn', 'ddzsingle_robot', __filename);
// Script/ComponentScript/component/ddzsingle_robot.js
...
ddz.AIRobot = cc.Class({
	...
	getCall: function getCall(seatIndex) {
		...
		if (this.called.length >= seatIndex) {
			//因为是命令叫分，所以不考虑重复让某玩家叫分的情况（已经叫过的话，get直接取值返回），一局游戏中此方法对一个seatIndex只会调用一次
		...
		if (this._isGrab) {
			...
			if (this.called[0] == 0) {
				...
				if (this.called.length == 3 && this.called[1] == 0 && this.called[2] == 0) {
					//必须用length来判断，否则undefined == 0
					this.dizhuPos = 1;
				}
			}
		} else {
			...
			if (seatIndex == 1) {
				...
				//左边玩家叫分，只要轮到左边玩家叫，一定叫3分,防止了都不叫开不了游戏
				call = 3;
				this.callMulti = 3;
				this.called[2] = call;
			}
	...
	playCards: function playCards(seatIndex, cards) {
		//某个座位的玩家出牌, 这里也不需要删除，因为在playcontroller会进行删除
	...
	genOneStep: function genOneStep(isInTrust) {
		var cards;
	...
	// 换牌
	exchangeCard: function exchangeCard(cardId) {
	...
	_findId: function _findId(cardId, array) {
		// 寻找牌的在数组中的下标
	...
	_passivityCards: function _passivityCards() {
		...
		if (types[0].getType() == hall.Enums.PaixingType.SHUNZI_CARD) {
			// V3.705产品要求其他两个玩家出顺子时的顺序为从左到右从大到小排列
  ...
  * 	false - 不可以转化
  */
	judgeType: function judgeType(arr, bLzChangable) {
		// bLzChangable 表示癞子牌是否需要转化。判断topcard的时候，癞子牌已经是其实际值，不需要转化。主动出牌选牌时，癞子可以转换成任意牌
	...
	startGame: function startGame() {
		//不要将底牌加入手牌，因为在playController里面, seatInfo.cards与这里的player[]是一个对象，两个地方都插入的话会重复
	...
	_sortByPoint: function _sortByPoint(o1, o2) {
		return o1.point > o2.point;
	},

	_getLaiziObject: function _getLaiziObject(dicarr) {
	...
	_getPointCountDicArray: function _getPointCountDicArray(arr) {
	...
	_getPCArrayByCount: function _getPCArrayByCount(arr) {
		//把数组number，转换成点数：牌数数组， 并将此数组按count从大到小排序
		var return_arr = this._getPointCountDicArray(arr);
		return_arr.sort(this._sortByCount);
		return return_arr;
	},

	_getPCArrayByPoint: function _getPCArrayByPoint(arr) {
		//把数组number，转换成点数：牌数数组， 并将此数组按point从小到大排序
		var return_arr = this._getPointCountDicArray(arr);
		return_arr.sort(this._sortByPoint);
		return return_arr;
	},

	_fillDicarr: function _fillDicarr(dicarr) {
	...
	_findMinSingle: function _findMinSingle(pLaizi, pcArray, count, ex, bDivide) {
	...
	//pcArray为已经转换好的point-count数组，按point排好序。count为数量， ex为需要排除的点数， 策略是从小到大找单牌，然后再拆牌
	_findMinDouble: function _findMinDouble(pLaizi, pcArray, count, ex, bDivide) {
		//如果pcArray是从小到大排列，则是从最小取。 如果是从大到小排列，则是从最大取
		...
		for (var i = 0; i < newarr.length; i++) {
			var t = newarr[i];
			if (t.count != 2 || bDivide && hall.GlobalFuncs.FindInArray(ex, t.point) >= 0 || t.point == 15) {
				//只有需要拆分的时候，才需要去判断ex
				continue;
			}
		...
		if (pLaizi != null && pLaizi.count > 0) {
			for (var i = 0; i < newarr.length; i++) {
				var t = newarr[i];
				if (t.point != 15 && t.count == 1 && (hall.GlobalFuncs.FindInArray(ex, t.point) < 0 || !bDivide)) {
					//寻找单牌
	...
	_findBomb: function _findBomb(pcArray, lz_obj) {
	...
	_findRocket: function _findRocket(pcArray) {
	...
	// 查找主动出牌的提示
	findActivePlayCards: function findActivePlayCards(array) {
		// 判断当前的手牌能否组合成一个有效牌型
	...
	// 查找手牌中的炸弹个数
	findBombResult: function findBombResult(card) {
		// _findBomb
		var result = [];
		var tmpArr = this._getPCArrayByPoint(array);
		var lz_obj = this._getLaiziObject(tmpArr);
  ...
  * array - 手牌
  */
	findWinCardsFromArray: function findWinCardsFromArray(type, array) {
	...
	_findWinSingle: function _findWinSingle(type, pc, lz_obj) {
	...
	//被动出牌：对子
	_findWinDouble: function _findWinDouble(type, pc, lz_obj) {
	...
	//拷贝一个p-c-c对象
	_copyOnePCObj: function _copyOnePCObj(dest, src) {
	...
	_copyPCArr: function _copyPCArr(dest, src) {
	...
	//被动出牌：三带
	_findWinThree: function _findWinThree(type, pc, lz_obj) {
	...
	// 'notBomb'		//不是炸弹
	_getBombType: function _getBombType(type) {
		// 不是4张，不可能是炸弹
  ...
  */
	_findWinBomb: function _findWinBomb(type, pc, lz_obj) {
	...
	//被动出牌
	////找到能管住type的顺子, 连对， 飞机
	_findWinContinous: function _findWinContinous(type, pc, lz_obj) {
		...
		for (var index = 0; index + continue_count <= newarr.length - 3; index++) {
			...
			for (var laiziMax = 0; laiziMax <= lz_obj.count; laiziMax++) {
				...
				if (cards.length > 0) {
					if (t_type == hall.Enums.PaixingType.AIRCRAFT_SINGLE_CARD || t_type == hall.Enums.PaixingType.AIRCRAFT_DOUBLE_CARD) {
						//找翅膀
	...
	_rocketWin: function _rocketWin(cards) {
	...
	_findAvailableAircraft: function _findAvailableAircraft(newarrpara, lz_obj) {
	...
	_findAvailableContinous: function _findAvailableContinous(newarr, cell_count, lz_obj) {
	...
	//找最大连对cell_count = 2, 顺子cell_count = 1， 不包括2和双王, 
	_findAvailableContinousDevide: function _findAvailableContinousDevide(newarr, cell_count, lz_obj) {
		ddz.LOGD('laizi count ' + lz_obj.count + ';  _findAvailableContinous, cell_count == ' + cell_count);
		// 默认是升序排列newarr，如果要找最大的可以倒序
		var return_arr = [];
		//index为起始搜索位置
	...
	_sortByLength: function _sortByLength(o1, o2) {
		return o1['len'] < o2['len'];
	},
	// 在此之前是检查连对、飞机、顺子等牌型
	_findAvailableThree: function _findAvailableThree(newarrpara, lz_obj) {
		var newarr = [];
		this._copyPCArr(newarr, newarrpara);
		var return_arr = [];
		var index;
	...
	//组合的种类数
	_getCombinatorialVariety: function _getCombinatorialVariety(n, r) {
		return 0;
	},
	...
	playActively: function playActively(seatIndex) {
		...
		if (this.dizhuPos == preIndex) {
			//只有地主是我上家，则敌人是上家，否则敌人都是下家
			enemyIndex = preIndex;
		} else {
...
var aitest = function aitest(cards, bLzChangable) {
...
var minGetTest = function minGetTest(cards) {
...
//被动出牌测试
var testPlayPassively = function testPlayPassively() {
	...
	var array = [
	//3,4,5,6,7,8,9,10
	countTable['CARD_D_8'], countTable['CARD_C_8'], countTable['CARD_D_9'], countTable['CARD_C_9'], countTable['CARD_S_9'], countTable['CARD_D_10'], countTable['CARD_C_10'], countTable['CARD_H_J'], countTable['CARD_S_J']];
...
//飞机带俩对相关测试
var testPlanAndTwoDouble = function testPlanAndTwoDouble() {
...
cc._RF.pop();
        ...
        //# sourceMappingURL=ddzsingle_robot.js.map
