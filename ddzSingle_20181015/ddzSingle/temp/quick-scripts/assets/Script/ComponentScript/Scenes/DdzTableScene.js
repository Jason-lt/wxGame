...
cc.Class({
                ...
                if (index > 1) {
                    for (var i = 0; i < index; i++) {
                        ...
                        action = cc.sequence(_delay, cc.callFunc(function () {
                            ...
                            if (i == index) {
                                that.playModeNode.runAction(cc.sequence(cc.delayTime(3), cc.callFunc(function () {
    ...
    onActionChat: function onActionChat(result) {
        ...
        if (result['fromseat'] == this._mySeatIndex) {
            if (result['toseat'] == ddz.GlobalFuncs.GetNextIndex(this._mySeatIndex)) {
                // 下家
    ...
    onActionGameReady: function onActionGameReady(result) {
        ...
        if (this.tableInfo().ftInfo) {
            ...
            if (this.tableInfo().ftInfo.curRound == 1) {
                //第一局,开局
                ty.BiLog.clickStat(ty.UserInfo.clickStatEventType.clickStatEventTypeStartFriendGame, [ddz.Share.shareKeywordReplace.inviteFriendID, ddz.Share.shareKeywordReplace.totalRound, ddz.Share.shareKeywordReplace.displayName, ddz.Share.shareKeywordReplace.goodCard]);
            }
        }
    ...
    onActionCardNote: function onActionCardNote(result) {
        if (result && result.isOpen == 1) {
            ...
            if (ddz.isClickJiPaiQi) {
                hall.MsgBoxManager.showToast({ "title": "首页观看广告可得记牌器哦~" });
            }
        }
        ddz.isClickJiPaiQi = false;
    },
        ...
        if (isFlow) {
            ...
            if (this.tableInfo().getSceneType() == ddz.Enums.SceneType.MATCH) {
                ...
                ty.Timer.setTimer(this, function () {
                    ...
                    if (ddz.matchModel.checkOldVersion() && ddz.matchModel.getStageIndex() === 1) {} else {
    ...
    getBaseScore: function getBaseScore() {
        ...
        } else if (sceneType == ddz.Enums.SceneType.MATCH) {
            //联网根据table_info获取，match和闯关待定
            base = table_config.m_base || 0;
        } else {
